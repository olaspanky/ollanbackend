...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    const order = new Order({
      ...
      prescriptionUrl,
      totalAmount,
      transactionNumber: customerInfo.transactionNumber, // Fixed reference
      paymentReference: `OLLAN_${Date.now()}_${Math.floor(Math.random() * 1000000)}`,
      status: 'pending',
    });
    ...
    // Invalidate Redis cache for admin orders
    try {
      await redisClient.del(`admin:orders:${user._id}`);
      logger.info(`Invalidated cache for admin: ${user._id}`);
    } catch (redisErr) {
      logger.error(`Failed to invalidate cache: ${redisErr.message}`);
    }
...
exports.getAdminOrders = async (req, res) => {
  ...
  try {
    ...
    if (!orders) {
      ...
      try {
        await redisClient.set(cacheKey, JSON.stringify(orders), 'EX', 3600); // Cache for 1 hour
        logger.info(`Orders cached in Redis for admin: ${adminId}, count: ${orders.length}`);
      } catch (redisErr) {
        logger.error(`Failed to cache orders in Redis: ${redisErr.message}`);
      }
    }
...
exports.updateDeliveryStatus = async (req, res) => {
  ...
  try {
    if (!['en_route', 'delivered'].includes(deliveryStatus)) { // Fixed: Use deliveryStatus instead of action
      return res.status(400).json({ message: 'Invalid delivery status' });
    }
...
// New endpoint to drop collection for testing (use with caution)
exports.dropOrderCollection = async (req, res) => {
  ...
  try {
    ...
    // Clear all Redis caches related to orders
    try {
      await redisClient.del(redisClient.keys('orders:*'));
      await redisClient.del(redisClient.keys('admin:orders:*'));
      logger.info('Cleared all order-related Redis caches');
    } catch (redisErr) {
      logger.error(`Failed to clear Redis caches: ${redisErr.message}`);
    }
