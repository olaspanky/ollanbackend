...
async function sendOrderNotification(order, status, additionalInfo = '') {
  try {
    ...
    // Add bank transfer details for pending orders
    let bankTransferInfo = '';
    if (status === 'pending') {
      bankTransferInfo = `
        Please make your payment to:
        Bank: Opay
        Account Name: Ollan Pharmacy Ltd
        Account Number: 7019312514
        Transaction Number: ${order.customerInfo.transactionNumber}
        Kindly ensure the transaction number is included with your payment for verification.
      `;
    }

    const fullAdditionalInfo = `${bankTransferInfo} ${additionalInfo}`.trim();
    await emailService.sendOrderStatusUpdate(email, customerName, orderId, status, fullAdditionalInfo);
  } catch (error) {
    logger.error('Email notification failed:', error);
  }
}
...
exports.createOrder = async (req, res) => {
  ...
  try {
    if (
      ...
      !customerInfo.email ||
      !customerInfo.phone ||
      !customerInfo.transactionNumber || // Add transactionNumber validation
      deliveryFee === undefined
    ) {
      return res.status(400).json({ message: 'All required fields must be provided' });
    }
    ...
    const order = new Order({
      ...
      customerInfo: {
        ...customerInfo,
        transactionNumber: customerInfo.transactionNumber, // Include transactionNumber
      },
      deliveryFee,
      prescriptionUrl,
      totalAmount,
      transactionNumber: customerInfo.transactionNumber, // Replace paymentReference
      status: 'pending',
    });
    ...
    // Send notification with bank transfer details
